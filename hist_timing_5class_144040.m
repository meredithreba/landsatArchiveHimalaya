...
breaktime_urban_from=zeros(5,28);
...
for k=1:r                  %go through all items in matrix.txt
    ...
    if k~=1 && classType==5 && pixelX==array(k-1,1) && pixelY==array(k-1,2)  %find the pixels that change into urban
        ...
        if last_time>726467 && last_time<736693 
            if array(k-1,6)==1 last_time_year=round(last_time/365); breaktime_urban_to(1,last_time_year-1990)=breaktime_urban_to(1,last_time_year-1990)+1; end
...
fid=fopen('/home/fas/seto/xl533/scratch60/grace1/within_from_to_time_144040.txt','w');
